...
// ============================================================================
// Permission hook relay (uses Claude Code's PermissionRequest hook)
// ============================================================================

/**
 * Generate a self-contained Node.js hook script that relays Claude Code
 * permission requests to the CV-Hub prompt API, polls for a response,
 * and returns the decision as structured JSON.
 */
...
// Auto-generated by cv-git agent — do not edit
const API = ${JSON.stringify(apiUrl)};
const TOKEN = ${JSON.stringify(token)};
const TASK_ID = ${JSON.stringify(taskId)};
const EXECUTOR_ID = ${JSON.stringify(executorId)};
...
async function main() {
  ...
  let desc = 'Claude Code wants to use tool: ' + toolName;
  if (toolName === 'Bash' && toolInput.command) {
    const cmd = toolInput.command.length > 200
      ? toolInput.command.slice(0, 200) + '...'
      : toolInput.command;
    desc = 'Claude Code wants to run: ' + cmd;
    if (toolInput.description) desc += '\\n(' + toolInput.description + ')';
  } else if ((toolName === 'Edit' || toolName === 'Write') && toolInput.file_path) {
    desc = 'Claude Code wants to ' + toolName.toLowerCase() + ': ' + toolInput.file_path;
  } else if (toolName === 'WebFetch' && toolInput.url) {
    desc = 'Claude Code wants to fetch: ' + toolInput.url;
  }

  process.stderr.write('[cv-agent] Permission relay: ' + desc.split('\\n')[0] + '\\n');

  // Also send a task log
  fetch(API + '/api/v1/executors/' + EXECUTOR_ID + '/tasks/' + TASK_ID + '/log', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
    body: JSON.stringify({ log_type: 'info', message: desc }),
  }).catch(() => {});

  // Create prompt in CV-Hub
  const res = await fetch(API + '/api/v1/tasks/' + TASK_ID + '/prompts', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...
      prompt_text: desc,
      options: ['Allow', 'Deny'],
      context: { source: 'claude_code_hook', tool_name: toolName, tool_input: toolInput },
      expires_in_minutes: 30,
    }),
  });

  if (!res.ok) {
    process.stderr.write('[cv-agent] Failed to relay prompt, auto-allowing\\n');
    output('allow');
    return;
  }

  const { id: promptId } = await res.json();
  process.stderr.write('[cv-agent] Prompt relayed, waiting for response...\\n');

  // Poll for response (30 min timeout, 3s interval)
  const deadline = Date.now() + 30 * 60 * 1000;
  while (Date.now() < deadline) {
    await new Promise(r => setTimeout(r, 3000));
    try {
      const pollRes = await fetch(API + '/api/v1/tasks/' + TASK_ID + '/prompts', {
      ...
      if (found && found.response) {
        const ans = found.response.toLowerCase().trim();
        const allowed = ['yes', 'y', 'allow'].includes(ans);
        process.stderr.write('[cv-agent] Response: ' + found.response + ' -> ' + (allowed ? 'allow' : 'deny') + '\\n');
        output(allowed ? 'allow' : 'deny');
        return;
      }
    } catch {}
  }

  process.stderr.write('[cv-agent] Prompt timed out, denying\\n');
  output('deny', 'Timed out waiting for approval');
}

function output(behavior, message) {
...
main().catch(() => output('allow'));
`;
}

/**
 * Set up a PermissionRequest hook before launching Claude Code.
 * Writes a temp script and configures .claude/settings.local.json.
 * Returns a cleanup function to call after the task completes.
 */
async function setupPermissionHook(
  creds: CVHubCredentials,
  executorId: string,
  taskId: string,
  cwd: string,
  ...
  const scriptPath = `/tmp/cv-agent-hook-${taskId}.mjs`;
  const settingsDir = path.join(cwd, '.claude');
  const settingsPath = path.join(settingsDir, 'settings.local.json');

  // Write hook script
  const script = generatePermissionHookScript(creds.CV_HUB_API!, creds.CV_HUB_PAT!, taskId, executorId);
  ...
  // Save original PermissionRequest hooks for restoration
  const originalPermHooks = settings.hooks.PermissionRequest || null;
  ...
  return async () => {
    ...
    try {
      if (existingSettings) {
        ...
        if (originalPermHooks) {
          existingSettings.hooks = existingSettings.hooks || {};
          existingSettings.hooks.PermissionRequest = originalPermHooks;
        } else if (existingSettings.hooks) {
          delete existingSettings.hooks.PermissionRequest;
...
async function launchClaudeCode(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.autoApprove) {
      ...
      if (isRoot) {
        // Can't use --dangerously-skip-permissions as root.
        // Permissions are handled via PermissionRequest hook (set up by caller).
        console.log(chalk.yellow('⚠') + ' Running as root — using PermissionRequest hook for approvals');
      } else {
...
async function executeTask(
  ...
  let hookCleanup: (() => Promise<void>) | null = null;

  try {
    console.log(chalk.cyan('🚀') + ' Launching Claude Code...');

    // Set up permission hook if running as root (can't use --dangerously-skip-permissions)
    const isRoot = process.getuid?.() === 0;
    if (options.autoApprove && isRoot) {
      try {
        hookCleanup = await setupPermissionHook(creds, state.executorId, task.id, options.workingDir);
        console.log(chalk.cyan('📡') + ' PermissionRequest hook installed for CV-Hub relay');
      } catch (err: any) {
        console.log(chalk.yellow('⚠') + ` Failed to set up permission hook: ${err.message}`);
      }
    }
    ...
    if (hookCleanup) await hookCleanup().catch(() => {});
    state.currentTaskId = null;

...
import { HOOK_TEMPLATES } from './init.js';
  ...
  cmd.action(async (options) => {
    try {
      ...
      if (options.fix) {
        ...
        // Rewrite stale hook scripts from current templates
        const hooksDir2 = path.join(process.cwd(), '.claude', 'hooks');
        await fs.mkdir(hooksDir2, { recursive: true });
        for (const [filename, getTemplate] of Object.entries(HOOK_TEMPLATES)) {
          const hookPath = path.join(hooksDir2, filename);
          try {
            const hookContent = await fs.readFile(hookPath, 'utf-8');
            const currentHash = createHash('sha256').update(hookContent).digest('hex').slice(0, 12);
            const expectedContent = getTemplate();
            const expectedHash = createHash('sha256').update(expectedContent).digest('hex').slice(0, 12);
            if (currentHash !== expectedHash) {
              await fs.writeFile(hookPath, expectedContent, { mode: 0o755 });
              fixed.push(`Updated ${filename} (${currentHash} → ${expectedHash})`);
            }
          } catch {
            // Hook doesn't exist — write it
            try {
              await fs.writeFile(hookPath, getTemplate(), { mode: 0o755 });
              fixed.push(`Installed missing ${filename}`);
            } catch { /* skip */ }
          }
        }
...
async function checkClaudeHooks(): Promise<DiagnosticResult> {
  ...
  // Check hook scripts use SESSION_ENV pattern (not old CLAUDE_ENV_FILE-only pattern)
  let usesSessionEnv = false;
  let hasHardcodedPaths = false;
  for (const f of ['session-start.sh', 'context-turn.sh', 'context-checkpoint.sh', 'session-end.sh']) {
    try {
      const hookContent = await fs.readFile(path.join(hooksDir, f), 'utf-8');
      if (hookContent.includes('SESSION_ENV=')) {
        usesSessionEnv = true;
      }
      if (hookContent.includes('/home/schmotz/')) {
  ...
  if (!usesSessionEnv) {
    return {
      name: 'Claude Code Hooks',
      status: 'warn',
      message: `${present.length}/4 hooks installed, but missing SESSION_ENV propagation pattern`,
      fix: 'Delete .claude/hooks/ and run "cv init -y" to reinstall production hooks',
    };
  }
  ...
  if (hasHardcodedPaths) {
    return {
      name: 'Claude Code Hooks',
      status: 'warn',
      message: `${present.length}/4 hooks installed, but contain hardcoded /home/schmotz paths`,
      fix: 'Delete .claude/hooks/ and run "cv init -y" to reinstall with dynamic paths',
    };
  }
  ...
  return {
    name: 'Claude Code Hooks',
    status: 'pass',
    message: `${present.length}/4 hooks installed, settings.json valid, SESSION_ENV propagation active`,
  };
}
...
/**
 * Check installed hooks match the expected templates.
 * Compares SHA-256 hashes for each hook file against the bundled templates.
 */
async function checkHookVersion(): Promise<DiagnosticResult> {
  ...
  if (stale.length === 0 && missing.length === 0) {
    return {
      name: 'Hook Version',
      status: 'pass',
      message: `All ${current.length} hooks up to date`,
    };
  }

  const parts: string[] = [];
  if (stale.length > 0) parts.push(`outdated: ${stale.join(', ')}`);
  ...
  return {
    ...
    status: 'warn',
    message: parts.join('; '),
    fix: 'Run "cv doctor --fix" or "cv init -y" to update hooks',
  };
}

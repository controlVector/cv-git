  ...
  cmd.action(async (options) => {
    try {
      ...
      if (options.fix) {
        ...
        // Fix stale hook scripts (missing SESSION_ENV or hardcoded paths)
        const hooksDir2 = path.join(process.cwd(), '.claude', 'hooks');
        for (const f of ['session-start.sh', 'context-turn.sh', 'context-checkpoint.sh', 'session-end.sh']) {
          const hookPath = path.join(hooksDir2, f);
          try {
            const hookContent = await fs.readFile(hookPath, 'utf-8');
            if (!hookContent.includes('SESSION_ENV=') || hookContent.includes('/home/schmotz/')) {
              await fs.unlink(hookPath);
              fixed.push(`Removed stale ${f} (will be regenerated by "cv init -y")`);
            }
          } catch { /* skip */ }
        }
